...
const ButtonComponent = ({ title, iconName, iconSize = 20, iconColor, type, onPress, style, round = false }) => {
  ...
  const baseButtonStyles = StyleSheet.create({
    button: {
      ...
      padding:  round ? 0 : spacing.size10,
    ...
    text: {
      ...
      marginLeft: iconName && !round ? spacing.size5 : 0, // Add margin only if there is an icon and it's not a round button
    },
    icon: {
  ...
  return (
    <TouchableOpacity onPress={onPress} style={[baseButtonStyles.button, style]}>
      {iconName && (
        <Ionicons name={iconName} size={iconSize} color={baseButtonStyles.icon.color} style={title ? { marginRight: spacing.size10 } : {}} />
      )}
